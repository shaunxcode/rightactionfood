(function() {
  ...
  $(function() {
    var RAF, User, appleCiderVinegar, basil, blsmq, bone, bunch, cannedTomatoes, cashews, chickpea, chive, clove, cup, eggplant, experience, garlic, greenOnion, heart, hunger, i, item, lemon, mushrooms, nutritionalYeast, onionPowder, oz, paprika, parsley, pepper, piece, quinoaLasagnaNoodles, recipe, recipes, sesameSeed, shallot, showModal, silkenTofu, spinach, stamina, sweetCorn, swissChard, tahini, tbsp, tomato, tsp, userDetailsModal, water, whiteBeans, whiteOnion, workspace, _i, _len;
    blsmq = {
      id: "blsmq",
        ...
        ingredients: [
          {
            ...
            amt: 1,
            unit: bunch,
            ingredient: swissChard,
            postDetail: ["rinsed", "chopped"]
          }, {
            amt: 1,
            unit: item,
            ingredient: tomato,
            ...
            amt: false,
            unit: false,
            ingredient: [paprika, nutritionalYeast, pepper],
            postDetail: "to taste"
          }
        ],
        ...
        ingredients: [
          {
            ...
            amt: 8,
            unit: oz,
            ingredient: quinoaLasagnaNoodles
          }, {
            amt: 0.5,
            unit: oz,
            ingredient: garlic
    ...
    RAF = {
      ...
      getItem: function(item) {
        var img;
        if (!(RAF.Items[item.name] != null)) {
          RAF.Items[item.name] = $("<div />").addClass("item").addClass(item.id).append($("<img />").attr({
            src: item.img
          }), $("<h3 />").text(item.name));
          img = new Image;
          img.onload = function() {
            return $("." + item.id + " img").attr((this.width > this.height ? "height" : "width"), 75);
          };
          img.src = item.img;
        }
        return RAF.Items[item.name].clone().data('itemId', item.id);
      }
    };
    RAF.Views.Recipe = (function(_super) {
      ...
      Recipe.prototype.className = "recipe";

      Recipe.prototype.render = function() {
        var img, ingredient, ingredientItems, ingredients, li, photo, photoClass, subingredient, _i, _j, _len, _len2, _ref, _ref2, _results;
        if (this.model.img.length === 0) {
        ...
        workspace.append(this.$el.append(photo = $("<div />").addClass(photoClass).append(img, $("<h3 />").text(this.model.name)), $("<h3 />").text("Ingredients"), ingredients = $("<ul />"), $("<h3 />").text("Directions"), $("<div />").addClass("instructions").text(this.model.directions), ingredientItems = $("<div />"), $("<div />").addClass("clear")));
        _ref = this.model.ingredients;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ingredient = _ref[_i];
          if (_.isArray(ingredient.ingredient)) {
            li = $("<li />");
            _ref2 = ingredient.ingredient;
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              subingredient = _ref2[_j];
              ingredientItems.append(RAF.getItem(subingredient));
              li.append($("<span />").text(subingredient.name), " ");
            }
            _results.push(ingredients.append(li));
          } else {
            ingredientItems.append(RAF.getItem(ingredient.ingredient));
            _results.push(ingredients.append($("<li />").append($("<span />").addClass("amt").text(ingredient.amt), " ", $("<span />").addClass("unit").text(ingredient.unit.name), " ", $("<span />").addClass("ingredient").text(ingredient.ingredient.name))));
          }
        }
        return _results;
      };
    ...
    $(".item").draggable({
      ...
      start: function(event, ui) {
        return ui.helper.css('zIndex', 10000);
      }
    });
    User = {
      items: {}
    ...
    $(".inventory").droppable({
      ...
      drop: function(event, ui) {
        ...
        if (!User.items[itemClass]) {
          $(".item." + itemClass).addClass("haveItem");
          User.items[itemClass] = true;
          $(this).find(".placeholder").remove();
          return $("<li></li>").html(ui.draggable.clone()).appendTo($("ul", this));
        }
      }
    });
    heart = $("<img />").attr({
      src: "http://www.minecraftwiki.net/images/thumb/a/a7/Heart.svg/120px-Heart.svg.png",
      width: 20
    }).addClass("heart");
    bone = $("<img />").attr({
      src: "http://www.minecraftwiki.net/images/thumb/6/65/Hunger.svg/120px-Hunger.svg.png",
      width: 20
    }).addClass("hunger");
    stamina = $(".stamina");
    hunger = $(".hunger");
    ...
    showModal = function() {
      return userDetailsModal.dialog('open');
    };
    return $(window).keypress(function(evt) {
      if (evt.charCode === 101) return showModal();
    });
  });
